...
public class BookActivityFeed extends AbstractActivityFeed<BookActivity> {
	...
	public String getTitle(BookActivity activity) {
		return messages.getMessages().format(getKeyForTitle(activity.getType().toString()), activity.getBook().getTitle(), activity.getUser().getUsername());
	}

	public String getSummary(BookActivity activity) {
		return messages.getMessages().format(getKeyForSummary(activity.getType().toString()), activity.getBook().getTitle(), activity.getUser().getUsername());
	}

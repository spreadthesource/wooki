        ...
        PublicationDAO
{
    ...
    public List<Publication> listPublication(Long chapterId)
    {
        ...
        Query query = session
                .createQuery(String
                        .format(
                                "select NEW %s(p.id, p.published, p.creationDate) from %s p where p.chapter.id=:chapter and p.chapter.deletionDate is null and p.deletionDate is null order by p.creationDate desc",
                                getEntityType(),
                                getEntityType()));

...
/**
 * Verify that the activation context passed corresponds to an existing activation method. Send 404
 * error if context method is not available.
 * 
 * @author ccordenier
 */
public class SecureActivationContextRequestFilter implements ComponentRequestFilter
{
            ...
            ComponentRequestHandler handler) throws IOException
    {
        ...
        if (manager.checkContext(parameters.getContainingPageName(), parameters
            ...
            ComponentRequestHandler handler) throws IOException
    {
        ...
        if (manager
                .checkContext(parameters.getLogicalPageName(), parameters.getActivationContext()))
        {
            handler.handlePageRender(parameters);
            return;
        }

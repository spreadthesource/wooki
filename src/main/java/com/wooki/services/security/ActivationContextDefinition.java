...
/**
 * Object used to store the information related to activation context for a given page.
 * 
 * @author ccordenier
 */
public class ActivationContextDefinition
{
    ...
    /**
     * This method must be called to check if the activation context of the current request
     * correspond to an existing matching method.
     * 
     * @param pageName
     * @param ctx
     * @return
    ...
    public boolean findMatchingMethod(String pageName, EventContext ctx)
    {
        ...
        if (unsecured) { return true; }

        // No activation context method inside class
        if (this.authorizedActivationContext.size() == 0) { return EmptyEventContext.class
                .equals(ctx.getClass()); }
        ...
        for (Class[] parameterType : this.authorizedActivationContext)
        {
            if (parameterType.length == ctx.getCount())
            {
                ...
                for (int i = 0; i < parameterType.length; i++)
                {
                    ...
                    {
                        ctx.get(type, i);
                        if (i == parameterType.length - 1) { return true; }
                    }
    ...
    private int getParameterCount(Class[] types)
    {
        ...
        if (types.length == 1)
        {
            ...
            if (soloType.equals(OBJECT_ARRAY_TYPE) || soloType.equals(EVENT_CONTEXT_TYPE)
                    || soloType.equals(LIST_TYPE)) return ANY_NUMBER_OF_PARAMETERS;
        }

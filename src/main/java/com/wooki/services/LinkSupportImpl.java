...
public class LinkSupportImpl implements LinkSupport {
	...
	public LinkSupportImpl(LinkSource linkSource, PageRenderLinkSource pageLinkSource, RequestPageCache pageCache, RenderSupport renderSupport) {
	...
	public BookMenuItem createEventMenuItem(LinkType type, String name, String pageName, String eventType, Object... context) {
		return this.createEventMenuItem(type, name, pageName, null, eventType, context);
	}

	public BookMenuItem createEventMenuItem(LinkType type, String name, String pageName, String nestedId, String eventType, Object... context) {
		Link link = this.linkSource.createComponentEventLink(pageCache.get(pageName), nestedId, eventType, false, context);
		return addToList(type, name, link);
	}

	public BookMenuItem createPageMenuItem(LinkType type, String name, String pageName, Object... pageActivationContext) {
		Link link = this.pageLinkSource.createPageRenderLinkWithContext(pageName, pageActivationContext);
		return addToList(type, name, link);
	}

	public BookMenuItem createNavLink(NavLinkPosition position, String name, String pageName, Object... context) {
	...
	public void commit(MarkupWriter writer) {
		...
		if (book != null) {
			...
			current = navBar.element("li", "id", "book-root", "class", "chapter-nav-item").element("h1");
	...
	private void addItems(List<BookMenuItem> items, Element root) {
		...
		for (BookMenuItem item : items) {
			...
			Element link = line.element("a", "href", item.getLink().toString(), "class", "bookmenuitem");
